...
from examples.vae_lsif import vae, q_net
...
if __name__ == "__main__":
    ...
    def log_joint(observed):
        model, _, _ = vae(observed, n, n_x, n_z, test_n_chains, False)
    ...
    def ais_log_prior(observed):
        z = observed['z']
        model, _, _ = q_net({'z': z}, x, n_z, test_n_chains, False)
        return model.local_log_prob('z')

    variational, _, _ = q_net({}, x, n_z, test_n_chains, False)
    qz_samples = variational.outputs('z')
    ...
    hmc = zs.HMC(step_size=1e-6, n_leapfrogs=test_n_leapfrogs,
                 ...
                 adapt_mass=True)
    bdmc = BDMC(ais_log_prior, log_joint, {'z': qz_samples}, hmc,
    ...
    with tf.Session() as sess:
        ...
        if ckpt_file is not None:
            ...
            for t in range(test_iters):
                ...
                time_iter += time.time()
                print('Test batch {} ({:.1f}s): lower bound = {}, '
                      'upper bound = {}'
