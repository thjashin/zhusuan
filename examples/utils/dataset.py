...
def imdb_raw_data(path='imdb.npz', num_words=None, skip_top=0, seed=113,
                  start_word=1, oov_word=2, index_from=3):
    """
    Loads the IMDB dataset.
    
    :param path: where to cache the data.
    :param num_words: max number of words to include. Words are ranked
        by how often they occur (in the training set) and only
        the most frequent words are kept.
    :param skip_top: skip the top N most frequently occurring words
        (which may not be informative).
    :param seed: random seed for sample shuffling.
    :param start_word: The start of a sequence will be marked with this word.
        Set to 1 because 0 is usually the padding word.
    :param oov_word: words that were cut out because of the `num_words`
    ...
    if not os.path.isfile(path):
        ...
        if not os.path.exists(os.path.dirname(path)):
            os.makedirs(data_dir)
        download_dataset('https://s3.amazonaws.com/text-datasets/imdb.npz',
                         path)

    with np.load(path) as f:
        x_train, labels_train = f['x_train'], f['y_train']
        x_test, labels_test = f['x_test'], f['y_test']
    print('x_train: length = {}, max_len = {}'.format(len(x_train),
                                                      max(map(len, x_train))))
    print('labels_train: class = {}'.format(len(set(labels_train))))
    print('x_test: length = {}, max_len = {}'.format(len(x_test),
                                                      max(map(len, x_test))))
    ...
    if oov_word is not None:
        xs = [[w if (skip_top <= w < num_words) else oov_word for w in x] for x
              in xs]
    else:
...
if __name__ == "__main__":
    imdb_raw_data('../data/imdb.npz', 20000)
